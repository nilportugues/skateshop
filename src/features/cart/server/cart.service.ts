...
import { deleteCartById, findCartById, updateCartItemsById } from "@/features/cart/server/db"
import { findProductById } from "@/features/product/server/db"

export async function updateCartItem({cartId, rawInput} : {cartId?: string, rawInput: z.infer<typeof cartItemSchema>}) {
...
export async function deleteCartItem({cartId, rawInput} : {cartId?: string, rawInput: z.infer<typeof deleteCartItemSchema>}) {
...
export async function deleteCartItems({cartId, rawInput} : {cartId?: string, rawInput: z.infer<typeof deleteCartItemsSchema>}) {
  if (!cartId) {
    throw new Error("cartId not found, please try again.")
  }
...
export async function createNewCartFromClosedCart({cartId, rawInput}: {cartId: string, rawInput: z.infer<typeof cartItemSchema>}) {
  const input = cartItemSchema.parse(rawInput)
...
export async function addToCartOrIncrementQuantity({cartId, rawInput}: {cartId: string, rawInput: z.infer<typeof cartItemSchema>}) {
  const input = cartItemSchema.parse(rawInput)
...
export async function createCartAndAddItems({rawInput}: {rawInput: z.infer<typeof cartItemSchema>}) {
  const input = cartItemSchema.parse(rawInput)
